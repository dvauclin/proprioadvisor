...
export const getLeads = async (): Promise<Lead[]> => {
  try {
    console.log('🔍 Starting getLeads for current user...');
    ...
    if (conciergerieError || !conciergerie) {
      console.log('❌ No conciergerie found for user:', user.email, conciergerieError);
      return [];
    }

    console.log('✅ Found conciergerie:', conciergerie.id);
    ...
    if (!formules || formules.length === 0) {
      console.log('❌ No formules found for conciergerie:', conciergerie.id);
      return [];
    }

    const formuleIds = formules.map(f => f.id);
    console.log('✅ Formule IDs for leads filtering:', formuleIds);
...
export const getAllLeads = async (): Promise<Lead[]> => {
  try {
    console.log('🔍 Admin getting all leads...');
    ...
    console.log('✅ Found', data.length, 'leads total');
