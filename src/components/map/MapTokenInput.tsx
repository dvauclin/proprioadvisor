...
const MapTokenInput: React.FC<MapTokenInputProps> = ({ onTokenSubmit }) => {
  return (
    <div className="p-6 border rounded-lg shadow-sm my-4">
      ...
      <p className="mb-4">Pour afficher la carte, veuillez entrer votre clé API publique Mapbox :</p>
      <div className="flex gap-2">
        <input 
          type="text" 
          className="flex-1 px-3 py-2 border rounded-md"
          placeholder="Entrez votre clé publique Mapbox" 
          onChange={(e) => {
      ...
      <p className="mt-2 text-xs text-gray-500">
        La clé sera enregistrée localement dans votre navigateur.
      </p>
    </div>
  );
};
