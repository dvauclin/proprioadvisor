...
export const fetchFormulesById = async (conciergerieId: string): Promise<Formule[]> => {
  ...
  try {
    ...
    console.log("Échantillon des formules disponibles:", allFormules?.map(f => ({
    ...
    console.log(`✅ ${formules?.length || 0} formules trouvées via requête directe pour ID: ${conciergerieId}`);
