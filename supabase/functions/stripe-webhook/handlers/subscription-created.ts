...
export async function handleSubscriptionCreated(subscription: Stripe.Subscription) {
  ...
  try {
    ...
    // Auto-validate conciergerie when subscription is created and payment is confirmed
    ...
    if (dbSubscription.monthly_amount > 0 && !conciergerie?.validated) {
      ...
      if (updateError) {
        ...
        logStep("Auto-validated conciergerie due to confirmed paid subscription", { 
  ...
  // Trigger webhook for subscription creation
